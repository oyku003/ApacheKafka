...
namespace Order.API.Services
{
    public class Bus(IConfiguration configuration, ILogger<Bus> logger ) : IBus
    {
        private readonly ProducerConfig _config = new()
        {
            ...
            AllowAutoCreateTopics = true,//defaultu truedur, daha önce topic oluşmadıysa kendi oluşturur.
        };
        ...
        public async Task CreateTopicOrQueue(List<string> topicOrQueues)
        {
            ...
            try
            {
                foreach (var topicOrQueue in topicOrQueues)
                {
                    await adminClient.CreateTopicsAsync(new[] { new TopicSpecification() { Name = topicOrQueue, NumPartitions = 6, ReplicationFactor = 1 } });

                    logger.LogInformation($"Topic {topicOrQueue} oluştu");
                }
            }
            catch (Exception e)
            {
